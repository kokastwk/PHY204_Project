...
L = 400e-10  # Length of the domain
T = 1e-14  # Total time
...
def f(u,x):
    E = 1e-10/(1.05e-34)
    coulomb = 2.3070775389e-28/(1.05e-34)
    if x == 0 :
        return 0
    else:
...
plt.xlabel('r')
plt.ylabel('u(r,T)')
plt.title('Numerical Solution at T='+str(k*T/N))
plt.show()'''
#print([sum([(np.linalg.norm(final_solution[k][i]))**2 for i in range(len(final_solution[k]))])*L/J  for k in range(len(final_solution))])
plt.figure(figsize=(10, 5))
plt.plot([q*T/N for q in range(len(final_solution))], [np.sum([((np.linalg.norm(final_solution[k][i]/(i*L/J)))**2)*L/J  for i in range(1,len(final_solution[k]))]) for k in range(len(final_solution))], linestyle='--')
plt.xlabel('r')
plt.ylabel('u(r,T)')
...
plt.figure(figsize=(10, 5))
plt.plot([q*T/N for q in range(len(final_solution))], [np.sum([((np.linalg.norm(final_solution[k][i]/(i)))**2)  for i in range(1,len(final_solution[k]))]) for k in range(len(final_solution))], linestyle='--')
plt.xlabel('r')
plt.ylabel('u(r,T)')
plt.title('Numerical Solution')
plt.show()
...
abs_line, = ax.plot(solver.x, np.abs(final_solution[0]), linestyle='--', label='Abs')
ax.set_xlabel('r')
ax.set_ylabel('u(r,T)')
ax.set_title('Numerical Solution at T=0')
ax.legend()

def update(k):
    abs_line.set_ydata(np.abs(final_solution[k]))
    ax.set_title('Numerical Solution at T={:.2f}'.format(k * T / N))
    return abs_line,

ani = FuncAnimation(fig, update, frames=kmax, interval=50, blit=True)
plt.show()
